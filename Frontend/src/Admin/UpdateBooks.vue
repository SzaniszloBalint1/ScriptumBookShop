<template>
    ...
    <Form @submit="onSubmit">
            ...
            <h1 class="m-5">Könyv frissítése</h1>
        ...
        <div class="form-group">
            ...
            <div v-if="!selectedCategory" class="error">Válassz egy kategóriát</div>
        </div>

        <button type="submit" class="btn btn-success submit-btn">Könyv frissítése</button>
    </Form>
    </div>  
</template>
...
export default {
    ...
    methods:{ 
        ...
        goToItemDetail(id){
            return this.$router.push(`/categories/${id}`);
        },
        async getBookData() {
            try {
                const book = this.$store.getters['book/bookId'];
                if (book) {
                    ...
                    this.describe = book.describe;
             
                    
                    if (book.category_id && this.categories) {
                        this.selectedCategory = this.categories.find(cat => cat.id === book.category_id);
                    }
                }
            } catch (error) {
                console.error("Hiba a könyv adatainak betöltésekor", error);
            }
        },
        async onSubmit(){
            ...
            try {
                ...
                const success = await this.$store.dispatch('book/updateBook', FormData);
                if (success) {
                    console.log("sikeres frissítés", this.$store.state.book);
                    this.statusSuccess = true;
                    this.statusMessage = 'Sikeres frissítés!';
                    setTimeout(() => {
                        this.$router.push('/admin');
                    }, 2000);
                }
            } catch (error) {
                console.log("sikertelen frissítés", error);
                this.statusSuccess = false;
                this.statusMessage = 'Sikertelen frissítés!';
            }
        },
    },
    created() {
